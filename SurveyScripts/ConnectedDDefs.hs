...
gParseModule str = do
  ...
  case parsed of
    ParseOk (Module a b c _ _ f decls) ->
      ...
      in
      return $ Left
                (zip modules cauterizedCDefs)
                 -- (zip modules ghostbusterDDefs) -- RRN HACKS
...
outputCCs onlyGADTs _hdl outputBase input =
  go `catch` \e ->
